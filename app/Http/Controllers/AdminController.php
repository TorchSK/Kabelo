...
class AdminController extends Controller
{
    ...
    {   
        ...
        $sitemap = $sitemap."\t".'<url>'."\n"."\t"."\t".'<loc>https://dedraslovakia.sk</loc>'."\n"."\t"."\t".'<priority>1.00</priority>'."\n"."\t".'</url>'."\n";

        foreach(Product::whereActive(1)->get() as $product)
        {
            $sitemap = $sitemap."\t".'<url>'."\n"."\t"."\t".'<loc>https://dedraslovakia.sk/p/'.$product->url.'</loc>'."\n"."\t"."\t".'<priority>0.80</priority>'."\n"."\t".'</url>'."\n";
        }

        foreach(Category::whereActive(1)->get() as $category)
        {
            $sitemap = $sitemap."\t".'<url>'."\n"."\t"."\t".'<loc>https://dedraslovakia.sk/'.$category->full_url.'</loc>'."\n"."\t"."\t".'<priority>0.90</priority>'."\n"."\t".'</url>'."\n";
        }

        foreach(Page::all() as $page)
        {
            $sitemap = $sitemap."\t".'<url>'."\n"."\t"."\t".'<loc>https://dedraslovakia.sk/'.$page->url.'</loc>'."\n"."\t"."\t".'<priority>0.90</priority>'."\n".'</url>'."\t"."\n";
        }
