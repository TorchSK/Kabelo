<div class="ui fluid steps unstackable">
  <a class="step cart_delivery {{$context}} @if ($cart['delivery_method']==$delivery->id) completed active @endif" data-delivery_method="{{$delivery->id}}" data-id="{{$delivery->id}}" data-type="delivery" @if($cart['price'] < App\Setting::whereName('min_free_shipping_price')->first()->value || App\Setting::whereName('min_free_shipping_price')->first()->value==0) data-price="{{$delivery->price}}" @else data-price="0" @endif data-note="{{$delivery->note}}" data-name="{{$delivery->name}}" data-price="{{$delivery->price}}" data-icon="{{$delivery->icon}}" data-desc="{{$delivery->desc}}">
