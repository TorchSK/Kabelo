<div id="filterbar" class="horizontal" data-item="filterbar_wrapper" style="background-color: {{App\Setting::firstOrCreate(['name'=>'filterbar_wrapper_background-color'])->value}};">
    ...
    @foreach(App\Category::where('active',1)->orderBy('order')->get()->toTree() as $category)

    	<a href="{{route('slug',['category'=>$category->full_url])}}" class="item @if(isset($requestCategory) && ($category->id == $requestCategory->id || in_array($category->id,$requestCategory->ancestors->pluck('id')->toArray()))) active @endif" data-cat="{{$category->id}}">
