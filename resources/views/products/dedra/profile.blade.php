      ...
      @if($product->sizes->count() > 0)
        <div class="sizes">
          ...
          @foreach($product->sizes as $size)
            <div class="size @if($size->stock=='poslední kusy' || $size->stock=='PRODEJ UKONČEN') inactive @endif @if($size->size_code==$product->id) selected @endif" data-code="{{$size->size_code}}">{{$size->text}}</div>
          @endforeach
        </div>
        @endif
