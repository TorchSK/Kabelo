	 ...
	 <div class="labels">
		...
		@elseif ($product->image)
			@if($product->thumbnail_flag==1)
			 <img data-flickity-lazyload="{{url((string)$product->image->thumb)}}" data-src="{{url((string)$product->image->thumb)}}" class="ui image" alt="{{$product->code}}-{{$product->name}}" style="max-height: 187px; display: inline-block;"/>
			@else
