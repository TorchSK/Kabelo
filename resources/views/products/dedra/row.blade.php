<div class="item product @if(isset($grid) && $grid)grid @endif @if($product->active==0) inactive @endif" data-productid={{$product->id}} data-minqty="{{$product->priceLevels->min('threshold')}}">
	 ...
	 <div class="labels">
		...
		@elseif ($product->image)
			@if($product->thumbnail_flag==1)
			 <img src="{{$product->image->thumb}}" class="ui image" alt="{{$product->code}}-{{$product->name}}" style="max-height: 187px; display: inline-block;"/>
			@else
			 <img src="{{$product->image->path}}" class="ui image" alt="{{$product->code}}-{{$product->name}}" style="max-height: 187px; display: inline-block;"/>
           @endif
	@endif
	...
	@if(!isset($showdesc))
	<div class="desc">{{substr(trim($product->desc),0,10000)}}</div>
	@endif
